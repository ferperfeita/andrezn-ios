// ============================================================
// andreznSS iOS v2.0.0
// Personalización visual y de marca para andrezn.
// Motor basado en el código KellerSS aportado por el usuario.
// Créditos originales visibles en el código fuente: Keller, Samir y Katiau.
// Uso autorizado únicamente para análisis defensivo de evidencias.
// ============================================================

const DEVICE_LANG = (Device.language() || "pt").toLowerCase().substring(0, 2)
const SPEECH = {
  pt: {
    start:    "Analisando, aguarde o andreznSS terminar",
    half:     "Scanner em cinquenta por cento. Aguarde mais um pouco.",
    probe:    "Scanner em noventa por cento. Aguarde mais um pouco.",
    done:     "andreznSS finalizado. Analise os resultados com cuidado.",
  },
  en: {
    start:    "Analyzing, please wait for andreznSS to finish.",
    half:     "Scanner at fifty percent. Please wait a little longer.",
    probe:    "Scanner at ninety percent. Almost done.",
    done:     "andreznSS finished. Analyze the results carefully.",
  },
  es: {
    start:    "Analizando, espera que andreznSS termine.",
    half:     "Escáner al cincuenta por ciento. Espera un poco más.",
    probe:    "Escáner al noventa por ciento. Ya casi termina.",
    done:     "andreznSS finalizado. Analiza los resultados con cuidado.",
  },
}
...
function buildHTML(findings, netEntries, cheatAppFindings, knownCheatFindings, ipsFindings, ipsMeta, _unused, ghostAppFindings, proxyLoginFindings, filename) {
  ...
  let uptimeBg    = uptimeWarning ? "background:linear-gradient(90deg,#2a1000,#1a0800)" : "background:#180508"
...
<meta charset="utf-8">
<title>andreznSS iOS</title>
<style>
  * { box-sizing:border-box; margin:0; padding:0; }
  body { background:#030303; color:#f0e6e8; font-family:-apple-system,ui-monospace,monospace; font-size:13px; }

  /* HERO */
  .hero {
    background:linear-gradient(155deg,#240007 0%,#090203 48%,#030303 100%);
    border-bottom: 1px solid #351018;
  ...
  .hero::after {
    ...
    background:radial-gradient(circle, #ff173d14 0%, transparent 70%);
    border-radius:50%; pointer-events:none;
  }
  .hero-eyebrow {
    font-size:9px; letter-spacing:3px; color:#ff173d66;
    text-transform:uppercase; margin-bottom:8px;
  }
  .hero-name {
    font-size:32px; font-weight:800; color:#fff;
    letter-spacing:-0.8px; margin-bottom:5px; text-shadow:0 0 22px #ff173d55;
  }
  .hero-credits {
    font-size:11px; color:#74313f; letter-spacing:2.5px;
  ...
  .hero-name span { color:#ff173d; }
  .hero-file {
    font-size:10px; color:#8b626c; word-break:break-all;
    padding:7px 10px; background:#11090b;
    border-radius:7px; border-left:3px solid #ff173d44;
    margin-bottom:14px; line-height:1.5;
    text-align:left;
  }
  .hero-file strong { color:#ff6a82; }
  .hero-grid { display:grid; grid-template-columns:1fr 1fr; gap:8px; }
  .hg-card {
    background:#11090b; border-radius:8px;
    padding:9px 12px; border:1px solid #351018;
  }
  .hg-label { font-size:9px; color:#74505a; letter-spacing:1px; text-transform:uppercase; margin-bottom:3px; }
  .hg-val        { font-size:12px; color:#f5e4e7; }
  .hg-val.cyan   { color:#ff173d; font-weight:bold; font-size:14px; }
  ...
  .lang-btn {
    background:#11090b; border:1px solid #351018; border-radius:20px;
    color:#8b626c; font-size:10px; letter-spacing:1px; padding:4px 10px;
    cursor:pointer; font-family:inherit; transition:all 0.2s;
    text-transform:uppercase; font-weight:600;
  }
  .lang-btn:hover { border-color:#ff173d66; color:#ff8196; }
  .lang-btn.active { background:#2c0009; border-color:#ff173d; color:#ff173d; }
  ...
  .stat {
    flex:1; background:#11090b; border-radius:10px;
    padding:12px 6px; text-align:center; border:1px solid #351018;
  }
  .stat .num { font-size:28px; font-weight:bold; line-height:1; }
  .stat .lbl { font-size:9px; color:#74505a; margin-top:4px; letter-spacing:1px; text-transform:uppercase; }
  ...
  .section-header .sh-sub { font-size:10px; color:#74505a; margin-top:1px; }
  .section-header .sh-count {
    font-size:11px; font-weight:bold;
    padding:3px 10px; border-radius:20px;
  }
  .sh-critical .sh-icon { background:#2a0009; }
  .sh-critical .sh-title { color:#ff1744; }
  .sh-critical .sh-count { background:#2a0009; color:#ff1744; border:1px solid #ff174444; }
  ...
  .divider { height:1px; background:#351018; margin:20px 0; }

  /* CARDS */
  .card {
    background:#11090b; border-radius:12px;
    margin-bottom:12px; overflow:hidden;
    border:1px solid #351018; border-left:4px solid #333;
  }
  .card.critical { border-left-color:#ff1744; background:#170005; border-color:#2a0009; }
  ...
  .badge.critical { background:#2a0009; color:#ff1744; border:1px solid #ff174455; }
  ...
  .conns { font-size:10px; color:#74505a; }
  .card-domain {
  ...
  .row {
    ...
    border-top:1px solid #351018; align-items:flex-start;
  }
  .label { color:#74505a; min-width:65px; font-size:10px; padding-top:1px; flex-shrink:0; line-height:1.4; }
  .sub   { color:#57343c; font-size:9px; }
  .val   { color:#e8d6da; word-break:break-all; flex:1; font-size:11px; line-height:1.5; }
  .isp    { color:#ffbb00; }
  .reason { color:#ff8a80; }
  .rdns        { color:#ff9aae; font-style:italic; }

  .ghost-banner {
    background:linear-gradient(135deg,#0b0507,#070304);
    border:1px solid #ff173d44; border-radius:12px;
  ...
  .ghost-title  { font-size:12px; font-weight:bold; color:#ff7890; letter-spacing:0.5px; }
  .ghost-sub    { font-size:10px; color:#5b2631; margin-top:2px; }
  .ghost-count  { background:#0d0508; color:#ef4968; font-size:11px; font-weight:bold; padding:2px 8px; border-radius:10px; border:1px solid #ff173d44; align-self:flex-start; }
  .ghost-rows   { display:flex; flex-direction:column; gap:8px; margin-bottom:10px; }
  .ghost-row    { display:flex; justify-content:space-between; align-items:flex-start; gap:8px; background:#0c0507; border:1px solid #321019; border-radius:8px; padding:10px; }
  ...
  .ghost-bundle { font-size:11px; font-weight:bold; color:#ffc0cb; word-break:break-all; }
  .ghost-domains { display:flex; flex-wrap:wrap; gap:4px; }
  .ghost-domain { font-size:9px; background:#16070b; color:#d74b66; border:1px solid #3f252b; padding:1px 6px; border-radius:8px; }
  .ghost-more   { font-size:9px; color:#69444d; }
  .ghost-hits   { font-size:11px; font-weight:bold; color:#ef4968; }
  .ghost-label  { font-size:9px; color:#57343c; background:#0b0507; padding:2px 6px; border-radius:6px; border:1px solid #3f252b; }
  .ghost-hint   { font-size:9px; color:#5b2631; border-top:1px solid #1a1a33; padding-top:8px; }
  ...
  .ips-banner {
    background:linear-gradient(135deg,#1a0a1a,#160005);
    border:1px solid #43000e; border-radius:12px;
    padding:14px; margin-bottom:12px;
  }
  .ips-header {
  ...
  .ips-title { font-size:12px; font-weight:bold; color:#ff4565; letter-spacing:0.5px; }
  .ips-sub   { font-size:10px; color:#7a3342; margin-top:1px; }
  .ips-count {
    background:#2a0009; color:#ff4565; border:1px solid #ff456555;
  ...
  .ips-row { }
  .ips-row-critical { background:#200006 !important; border-color:#ff004455 !important; }
  .ips-row-vpn      { background:#0b0507 !important; border-color:#ff173d44 !important; }
  ...
  .ips-cat-critical  { background:#260008; color:#ff3388; border:1px solid #ff338855; }
  .ips-cat-vpn       { background:#0d0508; color:#ff637e; border:1px solid #ff637e55; }
  ...
  .ips-row {
    ...
    background:#1a001a; border:1px solid #31000b; border-radius:8px; padding:8px 10px;
  }
  ...
  .ips-bundle { font-size:11px; font-weight:bold; color:#ff91a4; word-break:break-all; }
  .ips-reason { font-size:10px; color:#9d5362; line-height:1.4; }
  .ips-version { font-size:9px; color:#6d4049; }
  .ips-badge {
    font-size:9px; font-weight:bold; padding:2px 7px; border-radius:10px;
  }
  .ips-badge.launched  { background:#260008; color:#ff3f5f; border:1px solid #ff3f5f55; }
  .ips-badge.installed { background:#002200; color:#44aa44; border:1px solid #44aa4455; }
  .ips-hint { font-size:9px; color:#6a2835; line-height:1.4; }
  ...
  .ff-tag   { font-size:9px; color:#74505a; min-width:100px; text-transform:uppercase; letter-spacing:0.5px; }
  ...
  .ff-session-num  { font-size:9px; color:#74505a; text-transform:uppercase; letter-spacing:0.5px; }
  ...
  .pre-domain     { font-size:12px; color:#f3e5e8; word-break:break-all; flex:1; }
  ...
  .http-banner { color:#ff1744; font-weight:bold; text-transform:uppercase; font-size:10px; }
  .none   { color:#57343c; }
  .bundle {
    display:inline-block; background:#11090b; border-radius:5px;
    padding:2px 6px; font-size:9px; color:#8b626c; margin:1px;
    word-break:break-all; border:1px solid #351018;
  }
  .domain-row { padding:3px 0; font-size:11px; color:#e8d6da; word-break:break-all; }
  .domain-badge {
    display:inline-block; font-size:9px; font-weight:bold;
  ...
  .andrezn-footer {
    margin:26px 0 8px;
    padding:16px 14px;
    border:1px solid #351018;
    border-radius:12px;
    background:linear-gradient(135deg,#160005,#080304);
    color:#9f707a;
    text-align:center;
    font-size:10px;
    line-height:1.6;
...
<div class="hero">
  ...
  <div class="hero-name">andrezn<span>SS</span></div>
  <div class="hero-credits">personalizado por <span class="credit-name">andrezn</span> &middot; motor original KellerSS</div>
  <div class="lang-bar">
...
<div class="content">
  ...
  <div class="summary">
    <div class="stat">
      <div class="num" style="color:#ff1744">${criticalCount}</div>
  ...
  <div class="andrezn-footer">
    <strong>andreznSS iOS</strong>
    Análisis defensivo de evidencias · tema negro y rojo<br>
    Personalización: andrezn · motor original: KellerSS
  </div>
...
var TRANSLATIONS = {
  pt: {
    eyebrow: "Scanner iOS",
    credits: "personalizado por andrezn · motor original KellerSS",
    fileLabel: "Arquivo:",
    start: "Início",
  ...
  en: {
    eyebrow: "iOS Scanner",
    credits: "customized by andrezn · original KellerSS engine",
    fileLabel: "File:",
    start: "Start",
...
(function() {
  ...
  const uiLang = DEVICE_LANG === "pt" ? "pt" : DEVICE_LANG === "en" ? "en" : "es"
  await wv.evaluateJavaScript(`setLang("${uiLang}")`)
  Speech.speak(S.done)
  await wait(1200)
...
async function main() {
  ...
  step1.message = "Ve a:\n\nAjustes → Privacidad y seguridad → Reporte de privacidad de apps\n\nActiva el reporte antes de jugar la partida.\n\nDespués de jugar, pulsa Compartir o Exportar y guarda el archivo .ndjson original en Archivos o iCloud."
  ...
  step2.message = "Ve a:\n\nAjustes → Privacidad y seguridad → Análisis y mejoras\n\nActiva las opciones de análisis necesarias. Después entra en Datos de análisis y selecciona el archivo más reciente que comience con \"xp_amp_app_usage_dnu\".\n\nAbre el archivo → Compartir → Guardar en Archivos."
  ...
  step3.message = "Ahora selecciona los dos archivos guardados.\n\nPuedes elegirlos en cualquier orden; andreznSS identifica automáticamente cada tipo.\n\n📋 App_Privacy_Report.ndjson\n📊 xp_amp_app_usage_dnu*.ips"
  ...
  notice2.message = "Selecciona el segundo archivo o continúa para analizar solamente el primero."
  ...
  if (type2 && type1 === type2) {
    ...
    a.message = type1 === "ndjson"
      ? "Los dos archivos parecen ser reportes de privacidad. Selecciona un xp_amp_app_usage_dnu*.ips como segundo archivo."
      : "Los dos archivos parecen ser datos de análisis. Selecciona un App_Privacy_Report.ndjson como primer archivo."
    a.addAction("OK")
    await a.present()
    return
  }
  ...
  if (!ndjsonContent) {
    let a = new Alert()
    a.title = "Falta el Reporte de privacidad"
    a.message = "El archivo App_Privacy_Report.ndjson es obligatorio.\n\nAjustes → Privacidad y seguridad → Reporte de privacidad de apps → Compartir"
    a.addAction("OK")
    await a.present()
    return
  }
  ...
  if (!validation.ok) {
    let a = new Alert()
    a.title = "Reporte de privacidad no válido"
    a.message = validation.reason + "\n\nExpórtalo desde: Ajustes → Privacidad y seguridad → Reporte de privacidad de apps → Compartir"
    a.addAction("OK")
    await a.present()
    return
  }
